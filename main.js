...
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            arslanLog(`Connection closed for ${number}. Reason Code: ${reason}`, 'warning');

            // Agar user ne WhatsApp se bot ko manually logout kar diya ho (401)
            if (reason === DisconnectReason.loggedOut || reason === 401 || errorMessage.includes('401')) {
                arslanLog(`Manual unlink detected for ${number}, deleting session data...`, 'error');
                ...
                fs.removeSync(path.join(__dirname, 'session', `session_${sanitizedNumber}`)); // Local folder bhi clean karna zaroori hai
                
                socket.ev.removeAllListeners();
                return;
            }

            // AUTO-RECOVERY: Har kism ke error (Timeout, Network Drop, 500) par bot khud ko zinda karega
            arslanLog(`Auto-recovering session for ${number} in 5 seconds...`, 'info');
            const sanitizedNumber = number.replace(/[^0-9]/g, '');
            
            // Purana dead socket remove karo
            activeSockets.delete(sanitizedNumber);
            socket.ev.removeAllListeners();
            
            await delay(5000); // 5 seconds delay before reconnect
        ...
        if (connection === 'open') { 
            arslanLog(`Connection stabilized for ${number} 🚀`, 'success'); 
        }
    });
}
...
async function arslanPair(number, res = null) {
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            // ANTI-SILENT FIXES (Optimized Configuration)
            connectTimeoutMs: 60000,
            defaultQueryTimeoutMs: 60000,
            keepAliveIntervalMs: 10000,
            retryRequestDelayMs: 5000,
            markOnlineOnConnect: true,
            syncFullHistory: false, // Prevents loading too much history and getting stuck on Heroku
            emitOwnEvents: true,
            generateHighQualityLinkPreview: true,
            browser: ['QADEER-AI-MINI', 'Safari', '1.0.0'], // Proper bot identity
            
            getMessage: async (key) => {
                const msg = await arslanStore.loadMessage(key.remoteJid, key.id);
                return msg?.message || undefined; // Fixed undefined fallback
            }
        });
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 QADEER-AI — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
